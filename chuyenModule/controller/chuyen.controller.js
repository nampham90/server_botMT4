...
const Spch00201UpdateStatusProcess = require("../process/spch00201Process/spch00201UpdateStatusProcess");
exports.updateStatus = async (req, res) => {
    try {
        const spch00201UpdateStatusProcess = new Spch00201UpdateStatusProcess(dbCon.dbDemo);
        await spch00201UpdateStatusProcess.start();
        const session = spch00201UpdateStatusProcess.transaction;
        req.body.userid = req.userID;
        let response = await spch00201UpdateStatusProcess.updateStatus(req.body,session);
        await spch00201UpdateStatusProcess.commit();
...
const Spch00201HoanthanhchuyenhangProcess = require('../process/spch00201Process/spch00201HoanthanhchuyenhangProcess');
exports.hoanthanhchuyenhang = async (req,res) => {
    try {
       const spch00201HoanthanhchuyenhangProcess = new Spch00201HoanthanhchuyenhangProcess(dbCon.dbDemo);
       await spch00201HoanthanhchuyenhangProcess.start();
       const session = spch00201HoanthanhchuyenhangProcess.transaction;
       req.body.userid = req.userID;
       let response = await spch00201HoanthanhchuyenhangProcess.hoanthanhchuyenhang(req.body, session);
       await spch00201HoanthanhchuyenhangProcess.commit();
       return  res.status(200).send(new Response(0,"Data sucess ", response));
    } catch (error) {
        return  res.status(200).send(new Response(1001,"Lỗi hệ thống ", error.message));
    }
