...
class Spkh00201ThanhtoanPorcess extends AbsProcess {
    constructor(dbcon) {
        super(dbcon)
    }

    async thanhtoan(data,session) {
    ...
    async process(db, data, session) {
        ...
        if(check) {
            ...
            await PNH.collection.updateMany({soHDTTCN: { $regex: new RegExp(data.sohdttcnkh + "$") }},{$set: {status01: 1,hinhthucthanhtoan: 4, ngaythanhtoan: new Date() }},{session});
            const ghinhatkyhethongProcess = new GhinhatkyhethongProcess(db);
            let datank = {
                "loaithongbao": 'system',
                "lydo": 'Thanh toán công nợ đơn số: ' +  data.sohdttcnkh,
                "iduser": data.userid,
                "hanhdong": "updateMany",
                "table": "phieunhaphang"
            }
...
module.exports = Spkh00201ThanhtoanPorcess